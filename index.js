...
// server.js (replace your current backend file with this)
const express = require('express');
const app = express();
const cors = require('cors');
const jwt = require('jsonwebtoken');
...
const corsOptions = {
  origin: [
    'http://localhost:5173',
    'https://summer-camp-learning-school-server-evo264714.vercel.app',
    'https://summer-camp-school-4a8e2.web.app'
  ],
...
app.patch('/users/remove-instructor/:id', async (req, res) => {
  try {
    ...
    app.get('/singleuserclass', verifyJWT, async (req, res) => {
      ...
      const result = await singleUserClassCollection.find({ email }).toArray();
      res.send(result);
    });

    app.delete('/singleuserclass/:id', async (req, res) => {
      const result = await singleUserClassCollection.deleteOne({
